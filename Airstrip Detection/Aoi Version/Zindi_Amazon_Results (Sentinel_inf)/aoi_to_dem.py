...
AOI_TIF = r"aoi\Sentinel_AllBands_Inference_2020_01.tif" # <-- change to your big AOI GeoTIFF
DEM_TIF = r"dem\DEM_2020_01.tif" # <-- YOUR DEM FILE PATH (will be created by the script)
...
def is_terrain_flat_enough(dem_data: np.ndarray, dem_nodata: Any,
    ...
    # slope (very simple: sobel magnitude)
    ...
    if not np.any(valid):
        ...
        print(f"[DEM] All pixels are invalid, flat percentage is 0.00%")
        return False
    ...
    with rasterio.open(aoi_path) as src:
        ...
        for top in range(0, H, step):
            ...
            for left in range(0, W, step):
                ...
                # to PNG (bands→RGB; assumes B,G,R are bands 1,2,3 or 2,3,4 etc. pick best three)
                bands = tile
                if bands.shape[0] >= 3:
                    # Heuristic: try [R,G,B] = bands[3,2,1] if exists, else [0,1,2]
                    if bands.shape[0] >= 4:
                        rgb = np.stack([bands[3-1], bands[2-1], bands[1-1]], axis=-1) # (h,w,3)
                    else:
                        rgb = np.stack([bands[0], bands[1], bands[2]], axis=-1)
                else:
                    # single band gray
                    rgb = np.repeat(bands[0:1].transpose(1,2,0), 3, axis=2)

                # stretch to 8-bit
                rgb8 = np.zeros_like(rgb, dtype=np.uint8)
                for c in range(3):
                    rgb8[:,:,c] = normalize_to_uint8(rgb[:,:,c])

                png_path = os.path.join(png_dir, f"tile_{count:06d}.png")
                Image.fromarray(rgb8, mode="RGB").save(png_path, "PNG")
    ...
    for t in tiles_info:
        ...
        for r in results:
            ...
            for (x1, y1, x2, y2), conf, cls_id in zip(xyxy, confs, clses):
                ...
                # NOTE: YOLO uses (x,y) = (col,row) in image pixels
                ax1 = float(left + x1)
                ay1 = float(top + y1)
                ax2 = float(left + x2)
                ay2 = float(top + y2)
...
def build_aoi_rgb_preview(aoi_path: str) -> Tuple[Image.Image, Affine, CRS]:
    ...
    if bands.shape[0] >= 3:
        if bands.shape[0] >= 4:
            rgb = np.stack([bands[2], bands[1], bands[0]], axis=-1) # try 1,2,3 as RGB
        else:
            rgb = np.stack([bands[0], bands[1], bands[2]], axis=-1)
    else:
        rgb = np.repeat(bands[0:1].transpose(1,2,0), 3, axis=2)
